...
if(collapse_to_eu == FALSE) {
  ...
  if(country_product_breakdown == TRUE) {
    #For this program only, we are currently testing for volatility at the overall country level (not country and product level flow), as if we were to do it
    #at the more granular country/product level then reporting would be too complex. Therefore just use the country level file even if we are looking at country
    #and product level info.
    input_file <- paste0("1) data/raw/RTS UK and Scotland - dest country breakdown - ", data_date, ".csv")
    
    if(single_country != FALSE) {
      #If we only want to look at the product breakdown for a particular country, replace the input file:
      input_file <- paste0("1) data/raw/RTS UK and Scotland - ", single_country, " product breakdown - ", data_date, ".csv")
    }
  }
}
...
check_volatility <- function(group_name,
  ...
  #if less than 3 points in the time series, skip this series
  if(nrow(test_data) < 3) {
...
print(paste0(perc_vol, "% of rows show volatile products"))

#Now pull out the volatile series, so that we can update the list of erratics:
if(country_breakdown == TRUE & single_country == FALSE) {
  partner_erratics <- filter(check_volat, volatile == "potentially volatile") %>% 
    select(partner, flowtype) %>% 
    distinct() %>%
    mutate(flow_partner = paste(partner, flowtype)) %>% 
    pull(flow_partner) 
  
  print(paste("Country with erratic trade:", partner_erratics))
  
  #output a list of the newly found erratics:
  writeLines(partner_erratics, paste0("3) outputs/new erratics - partners.txt"))
}

#in either of these cases we want to examine volatility by product not by country as the grouping variable:
if(country_breakdown == FALSE | single_country != FALSE) {
  new_erratics <- filter(check_volat, volatile == "potentially volatile") %>% 
    select(ProductCode) %>%
    distinct() %>% 
    pull(ProductCode) 
  
  #Update the erratics list:
  ...
  #output a list of the newly found erratics:
  writeLines(new_erratics, paste0("3) outputs/new erratics - products.txt"))
}

#now pull out the country or product name (depending on what level the code is being run)
